...
example(of: "concatmap") {
    ...
    _ = observable.subscribe(onNext: { string in print(string) })
}
...
//    }
    let observable = Observable.combineLatest([left, right]) { $0.joined(separator: " ") }
    let disposable = observable.subscribe(onNext: { value in print(value) })
    ...
    right.onNext("world")
    print("> Sending a another value to Right")
...
example(of: "zip") {
    ...
    let observable = Observable.zip(left, right) { (weather, city) in return "It's \(weather) in \(city)" }
    _ = observable.subscribe(onNext: { value in print(value) })
}

example(of: "withLatestFrom / sample") {
...
example(of: "switchLatest") {
    ...
    source.onNext(one)
    one.onNext("Same text from sequence one")
    two.onNext("Same text from sequence two")
